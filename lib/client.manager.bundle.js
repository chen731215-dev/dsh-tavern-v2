        ...
        if (html.indexOf('StatusPlaceHolderImpl') !== -1) {
          ...
          setTimeout(function () {
            try {
              ...
              var _pid = (!_sid && typeof getActivePresetId === 'function' ? getActivePresetId() : '') || '';
              var _qs = _sid ? ('?sessionId=' + encodeURIComponent(_sid))
                : (_pid ? ('?presetId=' + encodeURIComponent(_pid)) : '');
              fetch('/api/muv-table/tavern-card' + _qs).then(function (r) { return r.json() }).then(function (d) {
