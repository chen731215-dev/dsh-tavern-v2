...
function extractPngChara(file) {
    ...
    function panelHTML() {
      return [
        ...
        // 剧情选项（此前只有绑定、没有元素 —— 开关永远找不到，等于不存在）
        '  <div class="t-card">',
        '    <span class="t-card-title">🎭 剧情选项 <span class="t-card-desc">要求模型在回复结尾给出可选行动，并渲染成可点按钮</span></span>',
        '    <div class="t-row" style="margin-top:8px;align-items:center;justify-content:space-between">',
...
//       setInterval(updateMsgCount, 5000);
      ...
      loadWb();
      __tavernWbRefresh = loadWb;   // 窗口重新获得焦点时自动重同步
      installWbFocusHook();
