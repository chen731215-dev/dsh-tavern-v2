window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function parseJsonText(text) { return JSON.parse(text); }
      ...
      //   既不等于编辑目标、也不等于会话权威 —— 用户看到「团队测试」却在注入「足控天堂」，
      //   「看不出来自己绑了什么」就是从这里来的。
...
function extractPngChara(file) {
    ...
    function panelHTML() {
      return [
        ...
        //   把「⚠️ _足控天堂2」之类线索混进去会把预设改成一个莫名其妙的名字。
        ...
        '      <div style="font-size:12px;color:var(--dsw-alias-label-tertiary,#999);margin-bottom:6px">选择要删除的预设：</div>',
        ...
        // ── P0-3a：当前会话绑定（三态）─────────────────────────────
        // 三个概念必须一眼分得开（混在一起就是本次「足控天堂」事故的根源）：
        ...
        '    <div class="t-row" style="margin-top:8px;align-items:center;flex-wrap:wrap;gap:6px">',
        ...
        '    <div id="tavern-binding-legacy" style="display:none;margin-top:6px;padding:6px 10px;border-radius:6px;background:rgba(231,76,60,.12);color:#e74c3c;font-size:12px"></div>',
        '    <div class="t-row" style="margin-top:8px;gap:6px;flex-wrap:wrap">',
        '      <button id="tavern-binding-unbind" type="button" class="t-btn-secondary" style="color:#e74c3c;white-space:nowrap">🔓 解绑本会话</button>',
        ...
        '    </div>',
        '    <div class="t-row" style="margin-top:8px;align-items:center;gap:6px">',
        ...
        '  <div class="t-card" style="border-color:rgba(46,204,113,.4)">',
        ...
        '    <div class="t-row" style="margin-top:8px;gap:6px;flex-wrap:wrap">',
        '      <button id="tavern-scope-global" type="button">🌍 所有会话生效</button>',
        '      <button id="tavern-scope-session" type="button" class="t-btn-secondary">💬 仅当前会话</button>',
        '      <button id="tavern-scope-cwd" type="button" class="t-btn-secondary">📁 当前工作区</button>',
        '    </div>',
        '    <div id="tavern-scope-allow-title" style="margin-top:8px;font-size:12px;color:var(--dsw-alias-label-secondary,#ccc)">已放行：</div>',
        '    <div id="tavern-scope-allow-chips" style="display:flex;flex-wrap:wrap;gap:6px;margin-top:4px"></div>',
        '    <div id="tavern-scope-disable-title" style="margin-top:8px;font-size:12px;color:var(--dsw-alias-label-secondary,#ccc)">已排除：</div>',
        '    <div id="tavern-scope-disable-chips" style="display:flex;flex-wrap:wrap;gap:6px;margin-top:4px"></div>',
        '  </div>',
        ...
        '  <div class="t-card" style="border-color:rgba(255,107,157,.35)">',
        '    <span class="t-card-title">🔞 NSFW 成人模式 <span class="t-card-desc">开启后每轮强注入破限词；开关即时写入服务端，下一轮生效</span></span>',
        ...
        '      </label>',
        '      <span id="tavern-nsfw-status" style="font-size:11px;color:var(--dsw-alias-label-tertiary,#999)">关闭</span>',
        '    </div>',
        '  </div>',
        ...
        // ── 变量面板（等价小白X 的「变量管理」）──────────────────────
        // 数据源是 muv-engine 的运行时变量树（GET/POST /api/muv-engine/state），
        // 树形查看 + 叶子编辑 + 写回（merge）+ 初始值对照（initvarData）+ 导出 JSON。
        '  <div class="t-card" style="border-color:rgba(155,89,182,.35)">',
        '    <span class="t-card-title">🔢 变量面板 <span class="t-card-desc">当前会话的运行时变量（MVU/ERA）：可查看、编辑并写回；改坏可用「刷新」丢弃本地改动</span></span>',
        '    <div class="t-row" style="margin-top:6px;gap:6px;align-items:center;flex-wrap:wrap">',
        '      <input id="tavern-var-search" type="text" placeholder="🔍 搜索变量路径或值…" style="flex:1;min-width:140px">',
        '      <button id="tavern-var-refresh" type="button" class="t-btn-secondary t-btn-sm" title="重新从服务端读取（丢弃未保存的本地修改）">🔄 刷新</button>',
        '      <button id="tavern-var-save" type="button" class="t-btn-sm" title="把已编辑的叶子写回服务端（merge）">💾 保存修改</button>',
        '      <button id="tavern-var-export" type="button" class="t-btn-secondary t-btn-sm" title="把当前变量树复制为 JSON">📋 导出 JSON</button>',
        '    </div>',
        // ⏱ 按楼回看（探测式）：快照接口（muv-engine 提供）不可用就整行保持 display:none，不报错不占位
        '    <div id="tavern-var-snap-row" style="display:none;margin-top:6px;gap:6px;align-items:center;flex-wrap:wrap">',
        '      <span style="font-size:12px;color:#9b59b6">⏱ 按楼回看</span>',
        '      <select id="tavern-var-snap-select" style="flex:1;min-width:160px;padding:2px 6px;font-size:12px;background:var(--dsw-alias-bg-layer-1,#2a2a3e);color:var(--dsw-alias-label-primary,#eee);border:1px solid var(--dsw-alias-border-l2,#444);border-radius:4px"></select>',
        '      <button id="tavern-var-back-latest" type="button" class="t-btn-secondary t-btn-sm" style="display:none" title="退出回看，恢复显示最新变量树">回到最新</button>',
        '    </div>',
        '    <div id="tavern-var-status" class="t-status" style="margin-top:6px"></div>',
        '    <div id="tavern-var-tree" style="margin-top:6px;max-height:340px;overflow-y:auto;font-size:12px;line-height:1.9;font-family:ui-monospace,Consolas,monospace"></div>',
        '  </div>',
        ...
        // 旧会话（log 非空）也能用，新会话交给自动播种（agent/created 时机）。
        '  <div class="t-card" style="border-color:rgba(231,76,60,.35)">',
        '    <span class="t-card-title">📌 开场白 <span class="t-card-desc">把当前卡的开场白注入当前会话末尾（旧会话 / 自动播种没生效时用）</span></span>',
        '    <div class="t-row" style="margin-top:6px;gap:6px;align-items:center;flex-wrap:wrap">',
        ...
        '  <div class="t-card" style="border-color:rgba(52,152,219,.35)">',
        ...
        // ① 「套用」= 把内置模块包按 name merge 进当前预设的 presets.json（写文件，持久）。
        //    同名模块默认保留用户版本（只补缺的）；勾上「覆盖」才替换成增强版。
        //    名字含 变量/Variable/era/格式/format 的模块（变量组）**永远**只补不覆盖。
        '    <div class="t-row" style="margin-top:8px;gap:6px">',
        '      <button id="tavern-enhance-apply" type="button">✨ 套用通用增强模块包</button>',
        '      <label class="t-check"><input type="checkbox" id="tavern-enhance-overwrite"> 覆盖同名模块</label>',
        '    </div>',
        '    <div id="tavern-enhance-status" class="t-status" style="margin-top:6px"></div>',
        '  </div>',

        // ── 通用增强层：运行时注入开关 ──
        // ② 不改预设也能生效：服务端在装配 system prompt 的**最后一段**追加通用约束
        //    （防抢话 / 防全知优先级最高），位置即优先级，压过预设里较弱的同类规则。
        //    开关**默认关闭** —— 用户当前的效果绝不能被插件被动改变。
        '  <div class="t-card" style="border-color:rgba(39,174,96,.35)">',
        '    <span class="t-card-title">✨ 通用增强层·运行时注入 <span class="t-card-desc">不依赖具体预设也能保证下限：文风 / 防抢话 / 防全知 / 抗过拟合；开关即时写入服务端</span></span>',
        '    <div class="t-row" style="margin-top:8px;align-items:center;justify-content:space-between">',
        '      <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:13px">',
        '        <input type="checkbox" id="tavern-enhance-runtime" style="cursor:pointer;width:16px;height:16px">',
        '        <span>运行时注入（不动预设文件，下一轮生效）</span>',
        '      </label>',
        '      <span id="tavern-enhance-runtime-status" style="font-size:11px;color:var(--dsw-alias-label-tertiary,#999)">关闭</span>',
        '    </div>',
        '  </div>',
        ...
        '    <span class="t-card-title" id="tavern-advanced-toggle" style="cursor:pointer;user-select:none">⚙️ 高级功能 <span class="t-card-desc">记忆 / 关系网 / 故事背景 / NSFW</span> <span style="font-size:10px;color:var(--dsw-alias-label-tertiary,#888);margin-left:8px;transition:transform .2s;display:inline-block" id="tavern-advanced-arrow">▶</span></span>',
        '    <div id="tavern-advanced-body" style="display:none;margin-top:10px">',

        // （NSFW 成人模式 / 剧情选项两个开关已移到「世界书」卡片下方的常驻可见区域 ——
        //   原先放在这个默认折叠的卡片里，等于界面上没有这两个开关。）
...
'      </div>',
        ...
        '    </div>',
        '    <div class="t-row"><button id="tavern-writing-save" type="button">💾 保存违禁词</button></div>',
    ...
    function mountTavernManager(root) {
      ...
      // ★ 这里的 nsfw / plotOptions 必须与服务端的默认值保持一致，
      //   并且在加载服务端状态时同步回来 —— 它们以前是「只赋值给开关、从不回读」的，
      //   一旦被用在生成逻辑里就会写出与服务端实况不符的预设（issue #2 就是这样来的）。
      var state = { characters: [], worldbooks: [], presets: [], activePresetIdx: -1, extraPrompt: '', nsfw: false, plotOptions: true, storyBackground: '' };
      var serverAgentYml = '';
        var presetSearch = '';
      ...
      //   变化处防抖 POST 到 /api/tavern/state，服务端存进全局 state（与 enhanceRuntime 同一套机制）。
      ...
      function loadCurrent() {
        ...
        return fetch('/api/tavern/read?presetId=' + encodeURIComponent(pid)).then(function (r) { return r.json(); }).then(function (data) {
          ...
          if (presetStatus) {
            ...
            presetStatus.innerHTML = '✅ 当前预设：' + (data.presetName || '默认预设') + '<br><span style="font-size:11px;color:var(--dsw-alias-label-tertiary,#999)">🎭 角色卡：' + charCount + ' 个 | 📚 世界书：' + wbCount + ' 本（' + wbEntries + ' 条，启用 ' + wbEnabledEntries + ' 条）| ⚙️ 预设：' + presetCount + ' 个（启用 ' + presetEnabledEntries + ' 条）</span>';
          ...
          if (sc) sc.textContent = mode === 'allowlist' ? '白名单模式：默认不吃卡，只有列表里的会话/工作区生效。' : '全局模式：所有会话都会加载，除下方排除列表。';
          // ★ P0-5：白名单语义是「空 = 不放行」。这里先把「生效范围」如实显示出来，
          //   空名单的红字提示在下面拿到 allowSessions 之后再补（/api/tavern/read 不返回它）。
          ...
          return fetch('/api/tavern/state').then(function (r) { return r.json(); }).then(function (sdata) {
            ...
            if (ist && sdata.ok) {
              ...
              // ★ P0-5：白名单语义改为「空 = 不放行」（旧语义是「空 = 不限制」）。
              //   空名单时若还显示「✅ 预设直接注入中」，用户会以为开了却什么都没注入，
              //   所以这里必须给红字说明，并告诉用户怎么恢复。
              //   ⚠ 只写状态文案，绝不回写 allowSessions / allowCwds（那是会落盘的字段）。
              var allowSessionsNow = sdata.allowSessions || [];
              var allowCwdsNow = sdata.allowCwds || [];
              var emptyAllow = m === 'allowlist' && allowSessionsNow.length === 0 && allowCwdsNow.length === 0;
              var emptyWarn = '⚠️ 白名单为空：当前不向任何会话注入，请显式添加会话或工作目录（在下方「🎯 生效范围」卡一键开启）';
              if (emptyAllow) {
                ist.textContent = sdata.cardEnabled
                  ? emptyWarn
                  : '❌ 未注入：预设直接注入已关闭（且白名单为空）';
                ist.style.color = '#e74c3c';
                if (sc) {
                  sc.textContent = emptyWarn + '，或切到「全局模式」。';
                  sc.style.color = '#e74c3c';
                }
              } else {
                ist.style.color = '';
                ist.textContent = sdata.cardEnabled ? (m === 'allowlist' ? ('✅ 预设直接注入中（白名单）：仅 ' + cnt + ' 个工作区生效，AI 不翻硬盘') : ('✅ 预设直接注入中（全局）：所有会话生效' + (cnt ? '，排除 ' + cnt + ' 个工作区' : '') + '，AI 不翻硬盘')) : '❌ 未注入：预设直接注入已关闭';
              }
            }
          }).catch(function () {});
        }).catch(function (err) {
        ...
        return fetch('/api/tavern/save', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ agentYml: agentYml, presetYml: presetYml, presetId: curPresetId, characters: state.characters, worldbooks: state.worldbooks, presets: state.presets, activePresetIdx: state.activePresetIdx }) }).then(function (r) { return r.json(); }).then(function (data) {
          ...
          if (data.ok) {
            ...
            st.innerHTML = '✅ <b>保存成功！Agent 预设已生成</b><br>' +
              ...
              '配置大小：' + ymlSize + ' KB' +
              '</span><br>' +
              '<span style="font-size:11px;color:var(--dsw-alias-label-tertiary,#666)">👉 新开会话时，在顶部预设选择器选择「' + agentPresetName + '」即可开始聊天</span>';
...
//       setInterval(updateMsgCount, 5000);
        ...
        fetch('/api/tavern/prompt-stats').then(function (r) { return r.json() }).then(function (d) {
          ...
          if (d.last && d.last.at) {
            h += '<div style="margin-top:4px;opacity:.7">上次组装 ' + String(d.last.at).replace('T', ' ').slice(0, 19)
              + ' · 卡片 ' + d.last.card + ' / 世界书 ' + d.last.wb + ' / 事实修正 ' + d.last.edits + ' / 破限 ' + d.last.nsfw + ' 字符</div>';
          }
          box.innerHTML = h;
      ...
      function refreshToggleStates() {
        return fetch('/api/tavern/state').then(function (r) { return r.json(); }).then(function (d) {
          if (!d || !d.ok) return;
          state.nsfw = d.nsfwEnabled === true;
          state.plotOptions = d.plotOptions !== false;
          var nt = container.querySelector('#tavern-nsfw-enabled');
          if (nt) nt.checked = state.nsfw;
          var ns = container.querySelector('#tavern-nsfw-status');
          if (ns) {
            ns.textContent = state.nsfw ? '🔥 已开启（下一轮生效）' : '关闭';
            ns.style.color = state.nsfw ? '#ff6b9d' : '#999';
          }
      ...
      function renderRelationsGraph(relations) {
        ...
        edges.forEach(function (e) {
          ...
          line.addEventListener('click', function (ev) {
            ...
            html += '<div style="margin-top:10px;text-align:right"><span style="color:var(--dsw-alias-label-tertiary,#666);font-size:11px;cursor:pointer" onclick="this.parentElement.parentElement.style.display=\'none\'">点击空白处关闭</span></div>';
      ...
      function loadSessionPresets(forcePresetId) {
        ...
        return fetch(presetsUrl).then(function (r) { return r.json(); }).then(function (data) {
          ...
          groups.forEach(function (g) {
            ...
            gHead.innerHTML = '<span style="font-size:10px;display:inline-block;transition:transform .15s;' + (g.collapsed ? '' : 'transform:rotate(90deg);') + '">▶</span><span style="flex:1">' + g.label + '</span><span style="font-size:10px;color:var(--dsw-alias-label-tertiary,#888)">' + g.items.length + ' 个</span>';
          ...
          var sidDisplay = ''; // 酒馆面板不再依赖会话ID
            ...
            presetStatus.innerHTML = '✅ 当前编辑：' + (currentPreset ? currentPreset.name : '默认预设') + '　|　共 ' + (data.presets || []).length + ' 个预设可选<br><span style="font-size:11px;color:var(--dsw-alias-label-tertiary,#999)">🎭 角色卡：' + charCount2 + ' 个 | 📚 世界书：' + wbCount2 + ' 本（' + wbEntries2 + ' 条，启用 ' + wbEnabled2 + ' 条）| ⚙️ 预设：' + presetCount2 + ' 个（启用 ' + presetEnabled2 + ' 条）</span>';
          presetStatus.style.color = '#27ae60';
        }).catch(function () {
      ...
      */
      ...
      function refreshNsfwStatus() {
        fetch('/api/tavern/state').then(function (r) { return r.json(); }).then(function (data) {
          ...
          state.nsfw = data.nsfwEnabled === true;   // 与服务端实况保持一致，别让内存值漂移
      ...
      nsfwToggle.addEventListener('change', function () {
        ...
        fetch('/api/tavern/state', {
          ...
          // 旧实现只判断「请求没有抛错」，服务端返回 ok:false 时界面照样显示已开启，
          // 与实际注入状态不符。这里显式校验 ok，并说明生效时机。
          if (!d || !d.ok) throw new Error((d && d.error) || '保存失败');
          state.nsfw = enabled;
          nsfwStatus.textContent = enabled ? '🔥 已开启（下一轮生效）' : '已关闭（下一轮生效）';
      ...
      // ── 通用预设增强层（preset-forge）─────────────────────────────
      // ① 运行时注入开关：不改预设文件，服务端在装配 system prompt 的最后一段追加
      //    通用约束（防抢话 / 防全知优先级最高，放最后是为了压过预设里较弱的同类规则）。
      //    默认关闭 —— 用户当前的效果不能被插件被动改变，开与不开由用户决定。
      var enhanceRtToggle = container.querySelector('#tavern-enhance-runtime');
      var enhanceRtStatus = container.querySelector('#tavern-enhance-runtime-status');

      function refreshEnhanceRuntime() {
        if (!enhanceRtToggle) return;
        fetch('/api/tavern/state').then(function (r) { return r.json(); }).then(function (d) {
          if (!d || !d.ok) return;
          var on = d.enhanceRuntime === true;
          enhanceRtToggle.checked = on;
          if (enhanceRtStatus) {
            enhanceRtStatus.textContent = on ? '✅ 已开启（下一轮生效）' : '关闭';
            enhanceRtStatus.style.color = on ? '#27ae60' : '#999';
          }
        }).catch(function () {});
      }

      if (enhanceRtToggle) {
        enhanceRtToggle.addEventListener('change', function () {
          var enabled = enhanceRtToggle.checked;
          if (enhanceRtStatus) { enhanceRtStatus.textContent = '⏳ 保存中…'; enhanceRtStatus.style.color = '#f39c12'; }
          fetch('/api/tavern/state', {
            method: 'POST', headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ enhanceRuntime: enabled })
          }).then(function (r) { return r.json(); }).then(function (d) {
            // 与 NSFW 开关同一条纪律：显式校验 ok，服务端没写成功就不能显示成已开启
            if (!d || !d.ok) throw new Error((d && d.error) || '保存失败');
            if (enhanceRtStatus) {
              enhanceRtStatus.textContent = enabled ? '✅ 已开启（下一轮生效）' : '已关闭（下一轮生效）';
              enhanceRtStatus.style.color = enabled ? '#27ae60' : '#999';
            }
          }).catch(function () {
            enhanceRtToggle.checked = !enabled;
            if (enhanceRtStatus) { enhanceRtStatus.textContent = '❌ 保存失败'; enhanceRtStatus.style.color = '#e74c3c'; }
          });
        });
      }

      // ② 套用到当前预设：按 name 去重，同名默认保留用户版本（只补缺的）。
      //    服务端 merge 前会先备份 presets.json（presets.json.bak-<时间戳>），可回滚。
      var enhanceApplyBtn = container.querySelector('#tavern-enhance-apply');
      if (enhanceApplyBtn) {
        enhanceApplyBtn.addEventListener('click', function () {
          var est = container.querySelector('#tavern-enhance-status');
          var owEl = container.querySelector('#tavern-enhance-overwrite');
          var overwrite = !!(owEl && owEl.checked);
          var pid = getActivePresetId();
          if (!pid) {
            if (est) { est.textContent = '❌ 还没选择预设，请先在顶部选一个'; est.style.color = '#e74c3c'; }
            return;
          }
          if (est) { est.textContent = '⏳ 正在套用…'; est.style.color = '#f39c12'; }
          fetch('/api/tavern/preset/enhance', {
            method: 'POST', headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ presetId: pid, overwrite: overwrite, groupIndex: state.activePresetIdx })
          }).then(function (r) { return r.json(); }).then(function (d) {
            if (!d || !d.ok) throw new Error((d && d.error) || '套用失败');
            var s = d.stat || {};
            if (est) {
              est.innerHTML = '✅ 已套用：新增 ' + (s.added || 0) + ' 条，覆盖 ' + (s.overwritten || 0)
                + ' 条，保留 ' + (s.kept || 0) + ' 条（同名），保护 ' + (s.protectedKept || 0) + ' 条（变量组/格式，未动）'
                + '<br><span style="font-size:11px;color:var(--dsw-alias-label-tertiary,#999)">预设组：' + esc(d.groupName || '') + ' · 现共 ' + (d.total || 0)
                + ' 条 · 备份：' + esc(String(d.backup || '（无）').split(/[\\/]/).pop()) + '</span>';
              est.style.color = '#27ae60';
            }
            // 以磁盘为准重读：不然内存里那份旧模块数组会在下一次「保存预设」时盖回去
            loadCurrent(pid);
          }).catch(function (e) {
            if (est) { est.textContent = '❌ ' + (e.message || '套用失败'); est.style.color = '#e74c3c'; }
          });
        });
      }

      // ── P0-3b：预设真名 ─────────────────────────────────────────
      // 问题现场：presets.json 里 `preset-mt5ip9cc-t6josi` 的 name 是「团队测试」，
      //   真实内容却是「足控天堂」（description 里写着 `_足控天堂2`）。只显示 name ⇒
      ...
      (function initBindingPanel() {
        ...
        function findPreset(id) {
          for (var i = 0; i < presetList.length; i++) { if (presetList[i] && presetList[i].id === id) return presetList[i]; }
          return null;
        }
        ...
        async function loadBinding() {
          ...
          if (!s) {
            ...
            render();
            setStatus('⚠️ 未检测到当前会话（先发一条消息，再回来操作绑定）', 'warn');
            return;
          }
          try {
        ...
        if (elUnbind) {
          elUnbind.addEventListener('click', async function () {
            ...
            try {
              ...
              render();
              setStatus('✅ 已解绑本会话：此会话回到出生默认值，不再注入任何酒馆角色卡/世界书', 'ok');
            } catch (e) {
          ...
          elApply.addEventListener('click', async function () {
            ...
            try {
              ...
              setStatus('✅ 已应用到当前会话：' + presetIdentityText(findPreset(pid) || { id: pid })
                + (d.started ? '（该会话已开始，角色卡本体被 DSH 锁定，本次仅世界书/记忆跟随）' : ''), 'ok');
            } catch (e) {
      ...
      ;(function initScopePanel() {
        ...
        function renderChips(box, items) {
      ...
      // ── 变量面板（等价小白X 的「变量管理」）─────────────────────────
      // 数据面：muv-engine 的运行时变量树。
      //   GET  /api/muv-engine/state?sessionId=… → { ok, state:{ data:<树> } }（★ 变量在 state.data）
      //   POST /api/muv-engine/state { sessionId, merge:true, data:{…} } → 深合并写回
      // 初始值对照：/api/muv-table/tavern-card?sessionId=… 的 initvarData —— 与运行时值
      //   不同的叶子标「已覆盖」，让用户能区分"卡声明的初始值"和"运行时被改过的值"。
      // 设计约束（保守）：
      //   · 只做 merge 写回，不做删除（merge 语义删不了键；删除等有快照功能再说）。
      //   · 编辑先落在本地 edits 表，「保存修改」才写服务端；「刷新」丢弃本地修改。
      //   · 大树懒展开：对象节点点开才渲染子层，不一次性渲染上千节点。
      //   · messageKey 快照入口：探测失败就隐藏，不报错不假装有数据。
      // ★ 前面 initScopePanel 的 IIFE 以 `})()` 结尾且中间只隔注释 —— 这里必须有前导分号，
      //   否则 ASI 会把 `(function…` 拼接成对上一表达式返回值的调用，整个 bundle 求值即炸
      //   （症状：酒馆管理面板整页白屏，控制台报 "(intermediate value) is not a function"）。
      ;(function initVarPanel() {
        var varTree = container.querySelector('#tavern-var-tree');
        var varStatus = container.querySelector('#tavern-var-status');
        var varSearch = container.querySelector('#tavern-var-search');
        if (!varTree || !varStatus) return;

        var currentTree = null;        // 服务端最新变量树（data）
        var initialTree = null;        // 卡声明式初始值（initvarData，可缺）
        var edits = {};                // pathKey -> { value, type }（未保存的本地修改）
        var expanded = {};             // pathKey -> true（已展开的对象节点）
        var snapMode = false;          // ⏱ 回看模式：true 时整树只读，绝不写回
        var snapLabels = {};           // messageKey -> 展示标签（时间 + 键尾 8 位）
        var TYPE_MARK = { object: '📂', string: '🔤', number: '🔢', boolean: '🔘' };

        function pathKey(arr) { return arr.join('\u0001'); }
        function pathLabel(arr) { return arr.map(function (k) { return String(k); }).join('.'); }

        function setStatus(msg, color) {
          varStatus.innerHTML = msg;
          varStatus.style.color = color || '#999';
        }

        // 从初始值树里按路径数组找初值（找不到返回 undefined）
        function initialValueAt(arr) {
          if (!initialTree || typeof initialTree !== 'object') return undefined;
          var cur = initialTree;
          for (var i = 0; i < arr.length; i++) {
            if (!cur || typeof cur !== 'object' || !(arr[i] in cur)) return undefined;
            cur = cur[arr[i]];
          }
          return cur;
        }
        ...
        // 判定叶子是否与初始值不同（类型或值）→「已覆盖」标记
        function isOverridden(arr, value) {
          var iv = initialValueAt(arr);
          if (iv === undefined) return false;
          return JSON.stringify(iv) !== JSON.stringify(value);
        }

        function leafInputHtml(type, value, pk) {
          // ⏱ 回看模式：叶子不渲染输入框，只渲染只读文本（回看视图绝不写回）
          if (snapMode) {
            return '<span style="font-size:12px;color:var(--dsw-alias-label-secondary,#ddd);word-break:break-all">' + esc(String(value)) + '</span>';
          }
          if (type === 'boolean') {
            return '<label style="cursor:pointer"><input type="checkbox" data-var-bool="' + esc(pk) + '"' + (value ? ' checked' : '') + '> ' + (value ? 'true' : 'false') + '</label>';
          }
          var v = type === 'number' ? String(value) : String(value);
          return '<input type="text" data-var-edit="' + esc(pk) + '" value="' + esc(v).replace(/"/g, '&quot;') + '" style="flex:1;min-width:120px;padding:2px 6px;font-size:12px;background:var(--dsw-alias-bg-layer-1,#2a2a3e);color:var(--dsw-alias-label-primary,#eee);border:1px solid var(--dsw-alias-border-l2,#444);border-radius:4px">';
        }

        function leafRow(arr, value) {
          var type = typeOf(value);
          var pk = pathKey(arr);
          // 回看模式下不标「已覆盖」：对照的是当前卡的 initvar，对历史楼层语义错位
          var overridden = snapMode ? false : isOverridden(arr, value);
          var edited = edits[pk];
          // ★ 已编辑的叶子要回显**编辑后的值**：不然 change 触发重渲染时
          //   输入框会被重置回旧值（编辑还在 edits 表里，但界面上看不到了）。
          var displayValue = edited !== undefined ? edited.value : value;
          var displayType = typeOf(displayValue);
          var label = pathLabel(arr.slice(-1));
          var html = '<div data-var-leaf style="display:flex;align-items:center;gap:6px;padding:1px 0 1px '
            + (Math.min(arr.length - 1, 6) * 14 + 4) + 'px">'
            + '<span style="color:#9b59b6">' + esc(label) + '</span>'
            + (overridden ? '<span title="与卡声明的初始值不同" style="font-size:10px;color:#e67e22">已覆盖</span>' : '')
            + (edited !== undefined ? '<span style="font-size:10px;color:#27ae60">●未保存</span>' : '')
            + '<span style="flex:1"></span>'
            + '<span style="flex:2;display:flex;align-items:center;gap:6px">'
            + (displayType === 'null' || displayType === 'array'
              ? '<span style="font-size:11px;color:var(--dsw-alias-label-tertiary,#888)">' + esc(displayType === 'null' ? 'null' : '[数组 ' + displayValue.length + ' 项]') + '</span>'
              : leafInputHtml(displayType, displayValue, pk))
            + '</span></div>';
          return html;
        }

        function objectRow(arr, childCount, isExpanded) {
          var pk = pathKey(arr);
          var caret = isExpanded ? '▼' : '▶';
          return '<div data-var-node="' + esc(pk) + '" style="cursor:pointer;user-select:none;padding:1px 0 1px '
            + (Math.min(arr.length - 1, 6) * 14) + 'px">'
            + '<span style="color:var(--dsw-alias-label-tertiary,#888)">' + caret + '</span> '
            + '<span style="color:#3498db">' + esc(String(arr[arr.length - 1])) + '</span>'
            + ' <span style="font-size:10px;color:var(--dsw-alias-label-tertiary,#888)">{' + childCount + '}</span></div>';
        }

        // 渲染一棵子树（只渲染到已展开层级 —— 懒展开）
        function renderInto(box, node, arr) {
          var keys = Object.keys(node || {});
          keys.sort(function (a, b) {
            var ao = node[a] !== null && typeof node[a] === 'object' && !Array.isArray(node[a]);
            var bo = node[b] !== null && typeof node[b] === 'object' && !Array.isArray(node[b]);
            if (ao !== bo) return ao ? 1 : -1;   // 叶子在前，对象在后
            return a < b ? -1 : a > b ? 1 : 0;
          });
          for (var i = 0; i < keys.length; i++) {
            var k = keys[i], v = node[k];
            var arr2 = arr.concat([k]);
            var t = typeOf(v);
            if (t === 'object' && Object.keys(v).length > 0) {
              var pk = pathKey(arr2);
              var isOpen = !!expanded[pk];
              box.insertAdjacentHTML('beforeend', objectRow(arr2, Object.keys(v).length, isOpen));
              if (isOpen) {
                var sub = document.createElement('div');
                sub.setAttribute('data-var-sub', pk);
                renderInto(sub, v, arr2);
                box.appendChild(sub);
              }
            } else if (t === 'object' || (t === 'array' && v.length === 0)) {
              box.insertAdjacentHTML('beforeend', leafRow(arr2, t === 'array' ? v : {}));
            } else {
              box.insertAdjacentHTML('beforeend', leafRow(arr2, v));
            }
          }
        }
        ...
        function render() {
          if (!currentTree || typeof currentTree !== 'object') {
            varTree.innerHTML = '<div style="color:var(--dsw-alias-label-tertiary,#888);padding:8px">（当前会话还没有变量 —— 先和卡聊一轮，或导入带 initvar 的卡）</div>';
            return;
          }
          var q = (varSearch && varSearch.value || '').trim().toLowerCase();
          if (q) {
            // 搜索模式：命中路径/值的叶子拍平显示（限 200 条防炸）
            var hits = [];
            (function walk(node, arr) {
              ...
              for (var k in node) {
                var v = node[k];
                var arr2 = arr.concat([k]);
                if (v !== null && typeof v === 'object' && !Array.isArray(v)) walk(v, arr2);
                else {
                  var hay = (pathLabel(arr2) + ' ' + String(v)).toLowerCase();
                  if (hay.indexOf(q) >= 0) hits.push([arr2, v]);
                  if (hits.length >= 200) return;
                }
              }
            })(currentTree, []);
            if (!hits.length) { varTree.innerHTML = '<div style="color:var(--dsw-alias-label-tertiary,#888);padding:8px">没有匹配的变量</div>'; return; }
            var html = '<div style="font-size:10px;color:var(--dsw-alias-label-tertiary,#888);margin-bottom:4px">匹配 ' + hits.length + ' 条叶子' + (hits.length >= 200 ? '（已截断）' : '') + '：</div>';
            for (var i = 0; i < hits.length; i++) {
              html += '<div style="font-size:10px;color:var(--dsw-alias-label-tertiary,#666);word-break:break-all">' + esc(pathLabel(hits[i][0])) + '</div>' + leafRow(hits[i][0], hits[i][1]);
            }
            varTree.innerHTML = html;
            bindLeafInputs();
            return;
          }
          varTree.innerHTML = '';
          renderInto(varTree, currentTree, []);
          bindLeafInputs();
          bindNodeToggles();
        }

        function bindNodeToggles() {
          var nodes = varTree.querySelectorAll('[data-var-node]');
          for (var i = 0; i < nodes.length; i++) {
            nodes[i].addEventListener('click', (function (el) {
              return function () {
                var pk = el.getAttribute('data-var-node');
                expanded[pk] = !expanded[pk];
                render();
              };
            })(nodes[i]));
          }
        ...
        function bindLeafInputs() {
          var inputs = varTree.querySelectorAll('[data-var-edit]');
          for (var i = 0; i < inputs.length; i++) {
            inputs[i].addEventListener('change', (function (el) {
              return function () {
                var pk = el.getAttribute('data-var-edit');
                // 还原原始类型：找不到原叶型按字符串存
                var orig = findByPk(pk);
                var t = typeOf(orig);
                var val = el.value;
                if (t === 'number') { var n = Number(val); val = isNaN(n) ? val : n; }
                edits[pk] = { value: val, type: t === 'boolean' || t === 'number' || t === 'string' ? t : 'string' };
                render();
              };
            })(inputs[i]));
          }
          var boxes = varTree.querySelectorAll('[data-var-bool]');
          for (var j = 0; j < boxes.length; j++) {
            boxes[j].addEventListener('change', (function (el) {
              return function () {
                var pk = el.getAttribute('data-var-bool');
                edits[pk] = { value: el.checked, type: 'boolean' };
                render();
              };
            })(boxes[j]));
          }
        ...
        // 把 edits 表构造成嵌套对象（merge 写回用）
        function editsToPayload() {
          var payload = {};
          for (var pk in edits) {
            var arr = pk.split('\u0001');
            var cur = payload;
            for (var i = 0; i < arr.length - 1; i++) {
              if (!cur[arr[i]] || typeof cur[arr[i]] !== 'object') cur[arr[i]] = {};
              cur = cur[arr[i]];
            }
            cur[arr[arr.length - 1]] = edits[pk].value;
          }
          return payload;
        }
        ...
        function loadState(quiet) {
          ...
          if (!sid) {
            currentTree = null; initialTree = null; render();
            setStatus('⚠️ 未检测到当前会话 —— 打开一个对话后再看变量', '#e67e22');
            return;
          }
          if (!quiet) setStatus('⏳ 读取变量中…', '#f39c12');
          fetch('/api/muv-engine/state?sessionId=' + encodeURIComponent(sid))
            .then(function (r) { return r.json(); })
            .then(function (d) {
              if (!d || !d.ok) throw new Error((d && d.error) || '读取失败');
              currentTree = (d.state && d.state.data) || null;
              // 顶层默认展开一级：否则满屏都是折叠的 {N} 节点，叶子根本看不见
              if (currentTree && typeof currentTree === 'object') {
                for (var tk in currentTree) {
                  var tv = currentTree[tk];
                  if (tv !== null && typeof tv === 'object' && !Array.isArray(tv) && Object.keys(tv).length > 0) {
                    expanded[pathKey([tk])] = true;
                  }
                }
              }
              render();
              var n = 0;
              (function count(x) { if (x && typeof x === 'object') { for (var k in x) { n++; count(x[k]); } } })(currentTree);
              setStatus('✅ 已读取' + (n ? '（顶层 ' + Object.keys(currentTree).length + ' 个分支）' : '（空树）')
                + (Object.keys(edits).length ? ' · 有 ' + Object.keys(edits).length + ' 处未保存修改' : ''), '#27ae60');
              // 顺带探测快照接口（静默：不可用就整行隐藏）
              initSnapshots();
            })
            .catch(function (e) {
              currentTree = null; render();
              setStatus('❌ 读取变量失败：' + esc(e.message || String(e)), '#e74c3c');
            });
          // 初始值对照（可缺 —— 没有卡或没 initvar 就不标）
          fetch('/api/muv-table/tavern-card?sessionId=' + encodeURIComponent(sid))
            .then(function (r) { return r.json(); })
            .then(function (d) {
              var iv = d && (d.initvarData || (d.card && d.card.initvarData)) || null;
              initialTree = (iv && typeof iv === 'object') ? iv : null;
              render();
            })
            .catch(function () { initialTree = null; });
        }

        // ── ⏱ 按楼回看（探测式时间旅行）─────────────────────────
        // 数据源（C:\dsh-muv-engine 提供，本仓库只做 UI）：
        //   GET /api/muv-engine/state?sessionId=…&snapshots=1
        //     → { ok, snapshots: [{ key, at, data|null }] }
        //   GET /api/muv-engine/state?sessionId=…&messageKey=<key>
        //     → 响应形状与普通 GET 完全一致（{ ok, state: { data } }），键不存在 404 { ok:false }
        // 探测失败（404 / 网络错 / 快照为空）→ 整行保持隐藏，不报错不占位。
        // 铁律：回看视图绝不写回 —— 「保存修改」在回看模式下直接拒绝。
        function fmtSnapTime(at) {
          var n = Number(at);
          if (!(n > 0)) return '未知时间';
          var d = new Date(n);
        ...
        function initSnapshots() {
          ...
          try { sid = getCurrentSessionId() || ''; } catch (e) {}
          if (!sid) return;
          fetch('/api/muv-engine/state?sessionId=' + encodeURIComponent(sid) + '&snapshots=1')
            .then(function (r) { if (!r || !r.ok) throw new Error('snapshots-unavailable'); return r.json(); })
            .then(function (d) {
              var list = (d && d.ok && Array.isArray(d.snapshots)) ? d.snapshots : null;
              if (!list || !list.length) return;   // 快照为空 → 保持隐藏
              buildSnapRow(list);
            })
            .catch(function () { /* 探测失败 → 静默隐藏 */ });
        }

        function buildSnapRow(list) {
          var row = container.querySelector('#tavern-var-snap-row');
          var sel = container.querySelector('#tavern-var-snap-select');
          if (!row || !sel) return;
          snapLabels = {};
          var html = '<option value="">— 选择楼层回看 —</option>';
          for (var i = 0; i < list.length; i++) {
            var s = list[i] || {};
            var key = String(s.key || '');
            if (!key) continue;
            var tail = key.length > 8 ? key.slice(-8) : key;
            var label = fmtSnapTime(s.at) + ' · …' + tail + (s.data == null ? '（过大未存）' : '');
            snapLabels[key] = label;
            html += '<option value="' + esc(key) + '"' + (s.data == null ? ' disabled' : '') + '>' + esc(label) + '</option>';
          }
          sel.innerHTML = html;
          row.style.display = '';
          if (!sel._snapBound) {
            sel._snapBound = true;
            sel.addEventListener('change', function () { enterSnap(sel.value); });
            var backBtn = container.querySelector('#tavern-var-back-latest');
            if (backBtn) backBtn.addEventListener('click', function () { exitSnap(); });
          }
        }

        function enterSnap(key) {
          if (!key) { exitSnap(); return; }
          var sid = '';
          try { sid = getCurrentSessionId() || ''; } catch (e) {}
          if (!sid) return;
          setStatus('⏳ 正在拉取该楼快照…', '#f39c12');
          fetch('/api/muv-engine/state?sessionId=' + encodeURIComponent(sid) + '&messageKey=' + encodeURIComponent(key))
            .then(function (r) { if (!r || !r.ok) throw new Error('该楼快照不存在'); return r.json(); })
            .then(function (d) {
              if (!d || !d.ok || !d.state) throw new Error('该楼快照不存在');
              snapMode = true;
              edits = {};
              currentTree = (d.state && d.state.data) || null;
              // 回看树也默认展开一级（与最新视图同款），否则满屏折叠节点没法看
              if (currentTree && typeof currentTree === 'object') {
                for (var tk in currentTree) {
                  var tv = currentTree[tk];
                  if (tv !== null && typeof tv === 'object' && !Array.isArray(tv) && Object.keys(tv).length > 0) {
              ...
              setStatus('⏱ 正在回看 ' + esc(snapLabels[key] || '历史快照') + '，点「回到最新」恢复', '#3498db');
            })
            .catch(function (e) {
              setStatus('❌ 回看失败：' + esc(e.message || String(e)), '#e74c3c');
            });
        }
        ...
        var varSaveBtn = container.querySelector('#tavern-var-save');
        if (varSaveBtn) {
          varSaveBtn.addEventListener('click', function () {
            // ⏱ 回看模式拒绝写回：历史楼层的树是快照，merge 写回会把旧值盖到最新状态上
            if (snapMode) { setStatus('⛔ 正在回看历史楼层的变量（只读）—— 点「回到最新」退出后再修改', '#e67e22'); return; }
            var n = Object.keys(edits).length;
            if (!n) { setStatus('没有未保存的修改', '#999'); return; }
            var sid = '';
            try { sid = getCurrentSessionId() || ''; } catch (e) {}
            if (!sid) { setStatus('⚠️ 未检测到当前会话，写不回去', '#e67e22'); return; }
            setStatus('⏳ 写回 ' + n + ' 处修改…', '#f39c12');
            fetch('/api/muv-engine/state', {
              method: 'POST', headers: { 'content-type': 'application/json' },
              body: JSON.stringify({ sessionId: sid, merge: true, data: editsToPayload() })
            }).then(function (r) { return r.json(); }).then(function (d) {
              if (!d || !d.ok) throw new Error((d && d.error) || '写回失败');
              edits = {};
              setStatus('✅ 已写回 ' + n + ' 处（merge）—— 卡下一轮查询/推送就能拿到新值', '#27ae60');
              loadState(true);
            }).catch(function (e) {
              setStatus('❌ ' + esc(e.message || String(e)) + '（本地修改已保留，可重试）', '#e74c3c');
            });
          });
        }

        var varRefreshBtn = container.querySelector('#tavern-var-refresh');
        if (varRefreshBtn) varRefreshBtn.addEventListener('click', function () { edits = {}; exitSnap(); });

        var varExportBtn = container.querySelector('#tavern-var-export');
        if (varExportBtn) {
          varExportBtn.addEventListener('click', function () {
            if (!currentTree) { setStatus('还没有变量可导出', '#999'); return; }
            var text = JSON.stringify(currentTree, null, 2);
            function fallback() {
              var ta = document.createElement('textarea');
              ta.value = text; document.body.appendChild(ta); ta.select();
              try { document.execCommand('copy'); setStatus('✅ 已复制到剪贴板（' + text.length + ' 字符）', '#27ae60'); }
              catch (e) { setStatus('❌ 复制失败，可手动从控制台取（见 document 变量）', '#e74c3c'); }
              document.body.removeChild(ta);
            }
            if (navigator.clipboard && navigator.clipboard.writeText) {
              navigator.clipboard.writeText(text).then(function () {
                setStatus('✅ 已复制到剪贴板（' + text.length + ' 字符）', '#27ae60');
              }).catch(fallback);
            } else fallback();
          });
        }
        ...
        if (varSearch) {
          varSearch.addEventListener('input', (function () {
            var t = null;
            return function () { clearTimeout(t); t = setTimeout(render, 250); };
          })());
        }

        // 首次挂载自动读一次（静默失败，不打扰）
        setTimeout(function () { loadState(true); }, 600);
      })();
      ...
      (function initGreetingPanel() {
        ...
        function setStatus(msg, color) { status.innerHTML = msg; status.style.color = color || '#999'; }
        btn.addEventListener('click', function () {
          setStatus('⏳ 注入中…');
          ...
          fetch('/api/tavern/greeting/insert', {
            ...
            if (d && d.ok) {
              ...
              //   开场白楼（自动播种种的或上次手动注入的）⇒ 不再叠加第二条，这里不是报错。
      ...
      (function initGlobalRegexPanel() {
        ...
        function escAttr(s) {
          return esc(String(s)).replace(/"/g, function () { return '&quot;'; });
        }
...
if (autoProg) autoProg.textContent = '自动总结：' + (data.mem.autoEnabled ? '✅ 已开启' : '❌ 已关闭') + ' | 每 ' + (data.mem.autoEvery || 20) + ' 楼总结一次 | 已总结到第 ' + (data.mem.lastSeq || 0) + ' 楼';
    ...
    function initMessageEditor() {
      ...
      function getMessageContentEl(msgEl) {
        // 正文容器同样按模式匹配（见 findAiMessages 上的说明），不写死哈希。
        if (isMessageBody(msgEl)) return msgEl;
        try {
          var bodies = msgEl.querySelectorAll('[class*="_markdown_"]');
          if (bodies.length) return bodies[bodies.length - 1];
        } catch (e) {}
        var selectors = ['.markdown', '.prose', '.message-content'];
        for (var i = 0; i < selectors.length; i++) {
        ...
        if (html.indexOf('StatusPlaceHolderImpl') !== -1) {
          ...
          setTimeout(function () {
            try {
              ...
              //   会盖掉会话 ⇒ 用户实测在「瑟瑟提瓦特」的会话里看到「足控天堂」的状态栏。
      ...
      function showPresetPanel(bar) {
        ...
        function renderList(filter) {
          ...
          for (var g = 0; g < groups.length; g++) {
            ...
            gHead.innerHTML = '<span style="font-size:9px;color:var(--dsw-alias-label-tertiary,#666);display:inline-block;transition:transform .15s;' + (isOpen ? 'transform:rotate(90deg);' : '') + '">▶</span><span style="flex:1;">' + grp.label + '</span><span style="font-size:10px;color:var(--dsw-alias-label-tertiary,#666);">' + items.length + ' 个</span>';
            // 当前预设所在组加 ● 提示
