...
function extractPngChara(file) {
    ...
    function panelHTML() {
      return [
        ...
        '        <span>启用剧情选项</span>',
        '      </label>',
        '      <span id="tavern-plot-options-status" style="font-size:11px;color:#999">关闭</span>',
        '    </div>',
        '  </div>',
        ...
        '      </label>',
        '      <span id="tavern-enhance-runtime-status" style="font-size:11px;color:#999">关闭</span>',
        '    </div>',
        '  </div>',
      ...
      */
          ...
          fetch('/api/tavern/preset/enhance', {
            ...
            if (est) {
              est.innerHTML = '✅ 已套用：新增 ' + (s.added || 0) + ' 条，覆盖 ' + (s.overwritten || 0)
                ...
                + '<br><span style="font-size:11px;color:#999">预设组：' + esc(d.groupName || '') + ' · 现共 ' + (d.total || 0)
      ...
      (function initVarPanel() {
        ...
        function leafInputHtml(type, value, pk) {
          ...
          if (snapMode) {
            return '<span style="font-size:12px;color:#ddd;word-break:break-all">' + esc(String(value)) + '</span>';
          }
          if (type === 'boolean') {
        ...
        function leafRow(arr, value) {
          ...
          var html = '<div data-var-leaf style="display:flex;align-items:center;gap:6px;padding:1px 0 1px '
            ...
            + (displayType === 'null' || displayType === 'array'
              ? '<span style="font-size:11px;color:#888">' + esc(displayType === 'null' ? 'null' : '[数组 ' + displayValue.length + ' 项]') + '</span>'
        ...
        function objectRow(arr, childCount, isExpanded) {
          ...
          return '<div data-var-node="' + esc(pk) + '" style="cursor:pointer;user-select:none;padding:1px 0 1px '
            ...
            + '<span style="color:#888">' + caret + '</span> '
            + '<span style="color:#3498db">' + esc(String(arr[arr.length - 1])) + '</span>'
            + ' <span style="font-size:10px;color:#888">{' + childCount + '}</span></div>';
        }
        ...
        function render() {
          if (!currentTree || typeof currentTree !== 'object') {
            varTree.innerHTML = '<div style="color:#888;padding:8px">（当前会话还没有变量 —— 先和卡聊一轮，或导入带 initvar 的卡）</div>';
            return;
          }
          ...
          if (q) {
            ...
            if (!hits.length) { varTree.innerHTML = '<div style="color:#888;padding:8px">没有匹配的变量</div>'; return; }
            var html = '<div style="font-size:10px;color:#888;margin-bottom:4px">匹配 ' + hits.length + ' 条叶子' + (hits.length >= 200 ? '（已截断）' : '') + '：</div>';
            for (var i = 0; i < hits.length; i++) {
              html += '<div style="font-size:10px;color:#666;word-break:break-all">' + esc(pathLabel(hits[i][0])) + '</div>' + leafRow(hits[i][0], hits[i][1]);
            }
      ...
      (function initGlobalRegexPanel() {
        ...
        function doImport() {
          ...
          fetch(API, {
            ...
            if (errs.length > 3) html += '<br><span style="font-size:11px;color:#999">…共 ' + errs.length + ' 条错误</span>';
        ...
        function rowHtml(s) {
          ...
          return '<div class="t-item" data-gregex-row="' + escAttr(id) + '">'
            ...
            + '<span style="font-size:11px;color:#888">placement:' + esc(placeShorthand(s.placement)) + (s.promptOnly ? ' · 仅提示词' : '') + (s.markdownOnly ? ' · 仅显示' : '') + '</span>'
        ...
        function renderList() {
          if (apiAvailable === false) { degrade(); return; }
          if (!scripts.length) {
            listEl.innerHTML = '<div style="color:#888;padding:8px;font-size:12px">（还没有全局正则脚本 —— 粘贴 ST 导出的 JSON 或拖入 .json 文件后点「📥 导入」）</div>';
            return;
          }
