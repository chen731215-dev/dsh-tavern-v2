...
## 📦 v2.3.2 更新（2026-09-12）

- 🐛 **persona 配置键名适配**：`@deepseek-ai/dsh-persona` 自 `0.1.3-alpha.2` 起把 persona 文本字段由 `text:` 改名为 `prefix:`（`text:` 被 schema 静默忽略 → 组装 agent 时校验失败、预设完全无法使用）。生成 / 保存 / 解析三处已全部改用 `prefix:`，并兼容读取旧预设的 `text:`
- 🐛 **成人模式 / 剧情选项此前完全不生效**：本插件生成的预设带 `complete: true`，DSH 的 `system-prompt` 组装遇到 complete 段会把系统提示词压缩为「仅该段」，服务端 `tavern:nsfw` / `tavern:plotOptions` 等运行时注入**会被整体丢弃**。现在这两个开关会在「💾 保存并注入」时直接写进 persona prefix，才能真正到达模型
- ✨ **剧情选项开关补全**：面板新增「启用剧情选项」复选框（此前只有查询、没有渲染，开关永远找不到）
- 🐛 **选项渲染修复**：旧渲染器长期被 `if (false)` 禁用；按钮 `data-opt` 写的是下标（点击会发送 `"0"`/`"1"`）已改为选项正文；收集逻辑改为仅在出现「选项引导语」时启用、并只取引导语之后的编号行，不再误伤正文里的编号列表
- 🐛 **角色卡文本提取修复**：块结束判断改为按缩进比较，不再把 `complete:` / `includeRuntimeContext:` 当角色卡正文吞进来
- 🎨 「💾 保存预设」按钮更名为「💾 保存并注入」，与文档一致
...
- 🎬 **剧情选项** — AI 每次回复输出 3 个选项，点击继续（**需点「💾 保存并注入」后新开会话生效**）
- 🔥 **成人模式** — 强破限注入，需搭配成人词条预设使用（**需点「💾 保存并注入」后新开会话生效**）

> ⚠️ **为什么必须「保存并注入」？** 本插件生成的预设使用 `complete: true`（persona 即完整系统提示词）。
> DSH 的提示词组装遇到 complete 段时，会把最终系统提示词压缩为「仅该段」，其它插件的运行时注入一律被丢弃。
> 因此这两个开关的文案必须写进 persona prefix——也就是「保存并注入」这一步。
